...
int main(int argc, char *argv[])
{
	...
	for (i = 1; i <= n; ++i) {
		char buf[16];
		int2str(i, 16, buf);
		xhint_t k = xh_put(str, h, buf, &ret);
		l = strlen(buf) + 1;
...
int main(int argc, char *argv[])
{
	...
	for (k = xh_begin(h); k != xh_end(h); ++k) // explicitly freeing memory takes 10-20% CPU time.
